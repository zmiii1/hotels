...
<div class="page-content">
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Add New Room Add-on</h6>
                    
                    <form class="forms-sample" method="POST" action="{{ route('room-addons.store') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('normal_price') is-invalid @enderror" id="normal_price" name="normal_price" value="{{ old('normal_price') }}" min="0" step="1000">
                                    @error('normal_price')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('price_type') is-invalid @enderror" id="price_type" name="price_type" required>
                                        ...
                                        @foreach($priceTypes as $key => $type)
                                            <option value="{{ $key }}" {{ old('price_type') == $key ? 'selected' : '' }}>{{ $type }}</option>
                                        @endforeach
                                    </select>
                                    @error('price_type')
                                        <span class="text-danger">{{ $message }}</span>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('for_guests_type') is-invalid @enderror" id="for_guests_type" name="for_guests_type" required>
                                        ...
                                        @foreach($guestTypes as $key => $type)
                                            <option value="{{ $key }}" {{ old('for_guests_type') == $key ? 'selected' : '' }}>{{ $type }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3" id="guestCountContainer" style="display: none;">
                            <label for="guest_count" class="form-label">Number of Guests</label>
                            <input type="number" class="form-control @error('guest_count') is-invalid @enderror" id="guest_count" name="guest_count" value="{{ old('guest_count') }}" min="1">
                            @error('guest_count')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-2">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="packages[]" value="{{ $package->id }}" id="package{{ $package->id }}" {{ in_array($package->id, old('packages', [])) ? 'checked' : '' }}>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_prepayment_required" name="is_prepayment_required" {{ old('is_prepayment_required') ? 'checked' : '' }}>
                            ...
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_included" name="is_included" {{ old('is_included') ? 'checked' : '' }}>
                            ...
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_bestseller" name="is_bestseller" {{ old('is_bestseller') ? 'checked' : '' }}>
                            ...
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_sale" name="is_sale" {{ old('is_sale') ? 'checked' : '' }}>
                        ...
                        <button type="submit" class="btn btn-primary me-2">Create Add-on</button>
