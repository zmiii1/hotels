...
<div class="card">
    <div class="card-body">
        <form action="{{ isset($promoCode) ? route('backend.beach-tickets.promo-codes.update', $promoCode->id) : route('backend.beach-tickets.promo-codes.store') }}" method="POST">
            ...
            <!-- Hidden field untuk applies_to yang di-hardcode ke beach_tickets -->
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <select name="beach_preference" id="beach_preference" class="form-control">
                            <option value="all" 
                                {{ old('beach_preference', 
                                    isset($promoCode) && $promoCode->beachTickets->count() > 0 && 
                                    $promoCode->beachTickets->pluck('beach_name')->unique()->count() > 1 ? 'all' : 
                                    (isset($promoCode) && $promoCode->beachTickets->count() > 0 ? 
                                        $promoCode->beachTickets->first()->beach_name : 'all')
                                ) == 'all' ? 'selected' : '' }}>
                                All Beaches
                            </option>
                            <option value="lalassa" 
                                {{ old('beach_preference', 
                                    isset($promoCode) && $promoCode->beachTickets->count() > 0 && 
                                    $promoCode->beachTickets->where('beach_name', 'lalassa')->count() == $promoCode->beachTickets->count() ? 'lalassa' : ''
                                ) == 'lalassa' ? 'selected' : '' }}>
                                Lalassa Beach Only
                            </option>
                            <option value="bodur" 
                                {{ old('beach_preference', 
                                    isset($promoCode) && $promoCode->beachTickets->count() > 0 && 
                                    $promoCode->beachTickets->where('beach_name', 'bodur')->count() == $promoCode->beachTickets->count() ? 'bodur' : ''
                                ) == 'bodur' ? 'selected' : '' }}>
                                Bodur Beach Only
                            </option>
                        </select>
            ...
            <div class="form-group mb-3">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle"></i> If you select specific tickets below, the promo code will only apply to those tickets. If none are selected, it applies to all beach tickets.
                        </div>
                ...
                <small class="text-muted">Leave all unchecked to apply to all beach tickets.</small>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const discountTypeAppend = document.querySelector('.discount-type-append');
        const maxDiscountContainer = document.querySelector('.max-discount-container');
        
        function updateDiscountType() {
            if (discountTypeSelect.value === 'percentage') {
                discountTypeAppend.textContent = '%';
                maxDiscountContainer.style.display = 'block';
            } else {
                discountTypeAppend.textContent = 'Rp';
